...
class GeminiProvider(BaseLLMProvider):
    ...
    def __init__(self):
        ...
        self.model = (getattr(settings, "GEMINI_MODEL", None) or "gemini-3.5-flash").strip()
        self._health_cache: Optional[Dict[str, Any]] = None
        self._last_health_check_time: float = 0.0
        self.fallback_models = ["gemini-3.5-flash", "gemini-3.6-flash", "gemini-3.7-flash", "gemini-3-flash-preview"]
    ...
    async def get_available_models(self) -> List[str]:
        return ["gemini-3.5-flash", "gemini-3.6-flash", "gemini-3.7-flash", self.model]
        ...
        for model_name in candidates:
            ...
            for attempt in range(2):
                try:
                    async with httpx.AsyncClient(timeout=timeout_sec) as client:
                        ...
                        if res.status_code == 200:
                            ...
                            if "api key" in err_body or "credential" in err_body or "unregistered" in err_body:
                                raise RuntimeError("Gemini API key is not configured.")
                            logger.warning(f"Model '{model_name}' returned HTTP {res.status_code}: {res.text[:100]}. Trying fallback model.")
                            break
                        elif res.status_code == 429:
                            logger.warning(f"Model '{model_name}' rate limited (HTTP 429). Trying fallback model...")
                            await asyncio.sleep(0.4)
                            break
                        elif res.status_code in (500, 502, 503, 504):
                            logger.warning(f"Model '{model_name}' service error (HTTP {res.status_code}).")
                            if attempt == 0:
                    ...
                    logger.warning(f"Network error on '{model_name}': {net_err}")
                    if attempt == 0:
                        await asyncio.sleep(1.0)
                        continue
                    break
                    ...
                    logger.warning(f"Gemini call exception on '{model_name}': {e}")
                    break
        ...
        for model_name in candidates:
            ...
            for attempt in range(2):
                try:
                    ...
                    logger.warning(f"Gemini chat exception on '{model_name}': {e}")
                    break
